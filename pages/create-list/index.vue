...
export default {
  ...
  async created() {
    try {
      const res = await this.$axios.$get(
        `https://market.21baza.ru/api/mob_auditor/add_document/?store_cod=${this.$store.state.storehouse.data?.store_cod}&token=${this.$store.state.user?.data?.token}`
      )
  ...
  methods: {
    async closeAudit() {
      try {
        await this.$axios.$get(
          `https://market.21baza.ru/api/mob_auditor/close_document/?store_cod=${this.$store.state.storehouse.data?.store_cod}&token=${this.$store.state.user?.data?.token}`
        )
    ...
    async handleCode(code) {
      try {
        ...
        const product = await this.$axios.$get(
          `https://market.21baza.ru/api/mob_auditor/add_document/items/?store_cod=${
    ...
    async sendProductAmount(amount) {
      try {
        const res = await this.$axios.$get(
          `https://market.21baza.ru/mobi/revision/add_document/items/?documentId=${this.documentId}&qr=${this.productId}&amount=${amount}`
        )
    ...
    async closeRevision() {
      try {
        await this.$axios.$get(
          'https://market.21baza.ru/api/mobi/revision/close_document/?documentId=' +
            this.documentId
        )
